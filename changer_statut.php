...
if (!$trajet_id || !$action) {
    header("Location: mes_trajets.php?message=Paramètres manquants.");
    exit();
}
...
try {
    ...
    if (!$trajet) {
        header("Location: mes_trajets.php?message=Trajet introuvable ou non autorisé.");
        exit();
    }
...
mail($to, $subject, $body, $headers);
        ...
        header("Location: mes_trajets.php?message=Trajet démarré !");
        ...
        header("Location: mes_trajets.php?message=Trajet terminé. Les passagers vont recevoir une notification.");
    } else {
        header("Location: mes_trajets.php?message=Action inconnue.");
    }
