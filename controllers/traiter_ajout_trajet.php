...
$sql = "INSERT INTO trajets (user_id, chauffeur, depart, adresse_depart, destination, adresse_arrivee, date, prix, places, plaque_immatriculation, date_immatriculation, duree_minutes)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);

try {
    $chauffeur_nom = $_SESSION['prenom'] . ' ' . $_SESSION['nom'];
    $stmt->execute([$user_id, $chauffeur_nom, $depart, $adresse_depart, $destination, $adresse_arrivee, $date, $prix, $places, $plaque_immatriculation, $date_immatriculation, $duree_minutes]);
    header("Location: ../views/mes_trajets.php?message=Trajet ajouté avec succès.");
    exit();
